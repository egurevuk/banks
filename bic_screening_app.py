...
# ─────────────────────────────────────────────────────────────────────────────
# Bulk screening: fetch full Russian bank list, screen each, export to XLSX
# ─────────────────────────────────────────────────────────────────────────────

BIK_INFO_BASE_XML_URL = "https://bik-info.ru/base/base.xml"

# OpenSanctions bills €0.10 per successful /search request. We use this to
# estimate cost upfront so the user can decide whether to trigger a run that
# might cost hundreds of euros. Source:
# https://www.opensanctions.org/faq/api/metering/ (last updated 2026-07-13)
OS_PRICE_PER_SEARCH_EUR = 0.10

# Average number of /search calls per bank in the bulk flow. Each bank triggers:
#   - 1 BIK search (always)
#   - 0-1 SWIFT search (skipped when iban.ru has no SWIFT for the BIC)
#   - 0-1 INN search (skipped when INN shape validation fails)
#   - +0-2 extra BIK searches for SBERBANK/VTB name matches (head-office
#     injection expands the BIK pool for regional branches)
# In practice most banks trigger all three, so ~3.0 is a defensible average.
OS_AVG_SEARCHES_PER_BANK = 3.0
...
VERDICT_ORDER: dict[str, int] = {
    VERDICT_MATCH: 0, VERDICT_REVIEW: 1, VERDICT_CLEAR: 2, VERDICT_ERROR: 3,
}


@st.cache_data(ttl=86400, show_spinner=False)
def fetch_bank_list_from_base_xml() -> list[dict[str, str]]:
    """Fetch and parse the full Russian bank list from bik-info.ru/base.xml.

    Returns list of ``{"bik": "...", "name": "...", "inn": "..."}`` dicts,
    deduped by BIK (which is the CBR registry's natural primary key). Only
    rows with a valid 9-digit numeric BIK are included; malformed entries
    are dropped silently.

    Defensive parser: bik-info.ru's XML schema isn't documented publicly,
    could express bank data either as element children
    (``<bank><bik>X</bik>...</bank>``) or as attributes (``<bank bik="X"
    .../>``). We walk every element in the tree, extract candidate fields
    from both attributes and children, and accept any element that yielded
    a BIK-shaped value. Namespace prefixes are stripped so schema variants
    with default xmlns still parse.

    Cached for 24h because the registry only changes when CBR licenses,
    revokes, or merges banks (handful of events per year).
    """
    try:
    ...
    def _extract_fields(elem: ET.Element) -> dict[str, str]:
        data: dict[str, str] = {}
        # Attributes — strip namespace prefixes
    ...
    for elem in root.iter():
        ...
        if len(bik) != 9 or not bik.isdigit() or bik in seen_biks:
            continue
        seen_biks.add(bik)
        # Promote full name to name if the short-name slot is empty
...
def screen_bank_simple(
    bik: str, name: str, inn: str, api_key: str
) -> dict[str, str]:
    """Headless single-bank verdict for the bulk XLSX flow.

    Mirrors the interactive Step 4 logic (whitelist → strict identifier search
    → OS-side whitelist filter → OFAC split) without UI side effects. Cached
    per-bank for 1h so a re-run within the hour costs nothing — important
    given the €0.10-per-call price of the underlying OpenSanctions requests.

    Returns ``{"verdict": VERDICT_*, "emoji": str, "detail": str}``.
    """
    try:
        whitelist = load_whitelist_swifts()
        wl_swifts = set(whitelist.keys())

        # 1. Resolve SWIFT via the cached iban.ru table (no extra HTTP call
        # per bank — the table is fetched once and shared across all screenings)
        ...
        # 2. Input-side whitelist check — if the resolved SWIFT is on
        # OhMySwift, short-circuit to CLEAR and skip all OS calls
        wl_hit = swifts_8 & wl_swifts
        if wl_hit:
            return {
                "verdict": VERDICT_CLEAR,
                "emoji": VERDICT_EMOJI[VERDICT_CLEAR],
                "detail": f"whitelisted: SWIFT {next(iter(wl_hit))} on OhMySwift",
            }
        ...
        if not api_key:
            return {
                ...
                "detail": "no OpenSanctions API key configured",
            }

        # 3. Strict identifier search — BIK (with SBERBANK/VTB head-office
        # injection), SWIFT (if known), INN (if valid)
        ...
        # 4. OS-side per-entity whitelist filter — matched entities whose
        # OS-stored swiftBic is on the OhMySwift list route to CLEAR too
        ...
        if not real_hits:
            if any_os_wl:
                return {
                    "verdict": VERDICT_CLEAR,
                    "emoji": VERDICT_EMOJI[VERDICT_CLEAR],
                    "detail": "whitelisted: OS-side matched entity SWIFT on OhMySwift",
                }
            return {
                "verdict": VERDICT_CLEAR,
                "emoji": VERDICT_EMOJI[VERDICT_CLEAR],
...
def generate_bulk_screening_xlsx(
    banks_with_verdicts: list[tuple[dict[str, str], dict[str, str]]],
) -> bytes:
    """Render the bulk screening results as an XLSX with color-tinted rows.

    Layout:
      - Sheet ``Summary``: verdict counts + source + generation timestamp
      - Sheet ``Screening``: full table, sorted MATCH → REVIEW → CLEAR → ERROR
        with row backgrounds tinted by verdict (light red / amber / green / grey)
    """
    ...
    rows = [
        ("Russian Banks — OpenSanctions Screening", ""),
        ("Source", "bik-info.ru/base.xml"),
        ("Generated (UTC)", datetime.utcnow().strftime("%Y-%m-%d %H:%M")),
        ("Total banks screened", str(len(banks_with_verdicts))),
        ("", ""),
        ("Verdict", "Count"),
    ]
    for label, value in rows:
        r = summary.max_row + 1 if summary.max_row > 1 or (summary.max_row == 1 and summary.cell(1, 1).value) else 1
        summary.cell(row=r, column=1, value=label)
        summary.cell(row=r, column=2, value=value)
    # Format the "Verdict / Count" header row
    verdict_header_row = summary.max_row
    for col in (1, 2):
        cell = summary.cell(row=verdict_header_row, column=col)
        cell.font = header_font
        cell.fill = header_fill

    for verdict_key in (VERDICT_MATCH, VERDICT_REVIEW, VERDICT_CLEAR, VERDICT_ERROR):
        r = summary.max_row + 1
        summary.cell(row=r, column=1,
                     value=f"{VERDICT_EMOJI[verdict_key]} {verdict_key}")
        summary.cell(row=r, column=2, value=counts.get(verdict_key, 0))
    summary.column_dimensions["A"].width = 32
    summary.column_dimensions["B"].width = 40

    # Title row: merge A1:B1 and give it bold styling
    ...
    columns = ["BIC", "Bank Name", "INN", "Verdict", "Detail"]
    for idx, col_name in enumerate(columns, start=1):
        c = ws.cell(row=1, column=idx, value=col_name)
        c.font = header_font
        c.fill = header_fill
        c.alignment = Alignment(horizontal="center", vertical="center")

    # Sort: most severe first (MATCH → REVIEW → CLEAR → ERROR)
    sorted_banks = sorted(
        banks_with_verdicts,
        key=lambda x: (VERDICT_ORDER.get(x[1]["verdict"], 99), x[0].get("bik", "")),
    )

    row_tint = {
        VERDICT_MATCH:  PatternFill("solid", start_color="FEE2E2"),  # light red
        VERDICT_REVIEW: PatternFill("solid", start_color="FEF3C7"),  # light amber
        VERDICT_CLEAR:  PatternFill("solid", start_color="DCFCE7"),  # light green
        VERDICT_ERROR:  PatternFill("solid", start_color="E5E7EB"),  # neutral grey
    }

    for row_offset, (bank, verdict) in enumerate(sorted_banks, start=2):
        values = [
            bank.get("bik", ""),
            bank.get("name", "") or "",
            bank.get("inn", "") or "",
        ...
        # Tint whole row by verdict
        tint = row_tint.get(verdict["verdict"])
        if tint is not None:
            for col_idx in range(1, len(columns) + 1):
                ws.cell(row=row_offset, column=col_idx).fill = tint

    ws.freeze_panes = "A2"
    # Autosize columns based on max content length in each column
    for col_idx, col_name in enumerate(columns, start=1):
        max_len = max(
            [len(str(col_name))]
            + [
                len(str(ws.cell(row=r, column=col_idx).value or ""))
                for r in range(2, len(sorted_banks) + 2)
            ]
        )
...
with st.sidebar:
    ...
    # ── Bulk screening: every Russian bank → XLSX ─────────────────────
    # Two-step gate protects against accidental clicks that would burn
    # hundreds of euros of OpenSanctions API budget. Step 1 fetches the
    # bank count from base.xml (free, cached), computes exact cost, and
    # reveals the "Confirm and run" button. Step 2 actually starts the
    # screening. The main-panel gate below runs the job.
    st.markdown("---")
    with st.expander("📊 Bulk screening (all Russian banks)", expanded=False):
        st.caption(
            "Fetches the full Russian bank list from "
            "[bik-info.ru/base.xml](https://bik-info.ru/base/base.xml), "
            "screens every BIC with the same verdict logic as the single-BIC "
            "flow, and returns an XLSX with **BIC · Bank Name · INN · "
            "Verdict · Detail** — sorted MATCH → REVIEW → CLEAR → ERROR, "
            "with rows tinted by verdict."
        )
        st.warning(
            "⚠️ **Cost warning.** OpenSanctions bills **€0.10 per "
            "successful `/search` request**. Each bank costs ~3 requests "
            f"(BIK + SWIFT + INN), so the full run of ~1,500 banks costs "
            f"approximately **€450**. Per-bank results are cached for 1h — "
            "a re-run within the hour is free. Click _Preview_ first to see "
            "the exact bank count and cost estimate before committing."
        )

        # Step 1: preview (free — cached base.xml fetch, no OS calls)
        if st.button("🔍 Preview cost", use_container_width=True):
            st.session_state["bulk_preview_shown"] = True

        if st.session_state.get("bulk_preview_shown"):
            with st.spinner("Fetching bank list from bik-info.ru…"):
                preview_banks = fetch_bank_list_from_base_xml()
            n_banks = len(preview_banks)
            if n_banks == 0:
                st.error(
                    "Couldn't fetch or parse base.xml. Try again in a "
                    "moment — this doesn't cost anything."
                )
            else:
                est_calls = int(n_banks * OS_AVG_SEARCHES_PER_BANK)
                est_cost_eur = est_calls * OS_PRICE_PER_SEARCH_EUR
                st.info(
                    f"📊 **{n_banks:,} banks** in base.xml.  \n"
                    f"Estimated API calls: **~{est_calls:,}** "
                    f"({OS_AVG_SEARCHES_PER_BANK:.0f}/bank average)  \n"
                    f"Estimated cost: **~€{est_cost_eur:,.0f}** "
                    f"(€{OS_PRICE_PER_SEARCH_EUR:.2f}/call)  \n"
                    "_Only successful HTTP 200 responses are billed. "
                    "Cached banks screened in a prior run within the "
                    "last hour cost nothing._"
                )
                if st.button(
                    f"▶ Confirm and screen all {n_banks:,} banks "
                    f"(~€{est_cost_eur:,.0f})",
                    use_container_width=True,
                    type="primary",
                ):
                    st.session_state["bulk_run"] = True
                    st.session_state["bulk_xlsx"] = None
                    st.session_state["bulk_preview_shown"] = False

        if st.session_state.get("bulk_xlsx"):
            st.success(
                f"XLSX ready — {st.session_state.get('bulk_count', '?')} banks "
                f"screened {st.session_state.get('bulk_when', '')}"
            )
            st.download_button(
                "⬇️ Download screening.xlsx",
                data=st.session_state["bulk_xlsx"],
                file_name="russian_banks_screening.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True,
            )

# ── Bulk screening main-panel gate ───────────────────────────────────────
# Runs when the user confirmed in the sidebar. Handled before the regular
# single-BIC UI so the user doesn't need to also enter a BIC. The final
# st.stop() below prevents the regular screening UI from rendering on top.
if st.session_state.get("bulk_run") and not st.session_state.get("bulk_xlsx"):
    st.markdown("## 📊 Bulk screening — in progress")
    st.caption(
        "Screening every Russian bank in bik-info.ru/base.xml with the same "
        "verdict logic as the single-BIC flow (whitelist → strict identifier "
        "search → OS-side whitelist → OFAC split). Per-bank results are "
        "cached for 1h so re-runs within the hour are free."
    )
    bulk_progress = st.progress(0.0)
    bulk_status = st.empty()
    bulk_status.info("Step 1/3 · Fetching bank list from bik-info.ru…")
    banks = fetch_bank_list_from_base_xml()
    if not banks:
        st.error(
            "Couldn't fetch or parse `https://bik-info.ru/base/base.xml`. "
            "No API calls were made. Try the sidebar button again in a moment."
        )
        st.session_state["bulk_run"] = False
        st.stop()
    bulk_status.info(
        f"Step 2/3 · Screening {len(banks)} banks — "
        f"cached ones are instant, uncached cost €{OS_PRICE_PER_SEARCH_EUR:.2f}/call…"
    )
    results: list[tuple[dict[str, str], dict[str, str]]] = []
    for i, bank in enumerate(banks):
        verdict = screen_bank_simple(
            bank["bik"], bank.get("name", ""), bank.get("inn", ""),
    ...
    bulk_status.info("Step 3/3 · Rendering XLSX…")
    try:
        xlsx_bytes = generate_bulk_screening_xlsx(results)
    except Exception as exc:
    ...
    from datetime import datetime
    st.session_state["bulk_xlsx"] = xlsx_bytes
    st.session_state["bulk_count"] = len(banks)
    st.session_state["bulk_when"] = datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")
    st.session_state["bulk_run"] = False

    bulk_progress.empty()
    bulk_status.success(f"✅ XLSX ready — {len(banks)} banks screened.")

    # Inline download in the main panel (the sidebar version appears on
    # next rerun, since the sidebar was rendered before the job started)
    st.download_button(
        "⬇️ Download screening.xlsx",
        data=xlsx_bytes,
        file_name=f"russian_banks_screening_{datetime.utcnow().strftime('%Y%m%d_%H%M')}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        type="primary",
    )
    ...
    summary = " · ".join(
        ...
        for k in (VERDICT_MATCH, VERDICT_REVIEW, VERDICT_CLEAR, VERDICT_ERROR)
        if counts.get(k, 0) > 0
    )
    st.markdown("**Verdict breakdown:** " + summary)
    st.caption(
        "_The download button also appears in the sidebar after your next "
        "interaction with the app. The XLSX stays available for the rest of "
        "this browser session._"
    )
    st.stop()
